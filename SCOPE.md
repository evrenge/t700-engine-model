...
| Whole-curve RMS, normalised by each panel's excursion | **8 % ratchet** | `test_whole_curve.WHOLE_CURVE_RMS_CEILING_PCT`. Not a tolerance -- set just above the worst measured panel so a regression fails and an improvement is free. Current: mean **3.51 %**, range 1.29-7.72 % over nine panels. It went 8 -> 14 -> 8 on 2026-09-13 and the round trip is worth recording. The raise: the P3/P41 loop was corrected to stop on the error the report states rather than the iterate step, Figure 9 improved on four of five panels, and Figure 10's chop degraded to t45 13.15 % with a floor of NGc 69.90 % -- attributed at the time to `f1`'s 65-80 %NGc data hole. The return: **it was not `f1`**. Eq. 80's fixed-point iteration does not converge where f9's elasticity is below -1, and that, not the compressor map, was letting the chop run away. Solving Eq. 80 when its printed iteration fails puts the floor at **74.08 %NGc against Ballin's 74.18** and t45 at **7.72 %**, with Figure 9 unmoved. `f1`'s hole is still open (#58) and is second-order |
